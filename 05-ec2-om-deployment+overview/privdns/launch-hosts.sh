...
echo "Starting instances and downloading agent from PRIVDNS; Project is $PROJECT_ID and agent key is $AGENT_API_KEY"

# start 3 hosts and install the agent
aws ec2 run-instances --image-id $IMAGE --count $NUM_HOSTS --instance-type t3a.xlarge --key-name $KEYNAME \
  ...
  --tag-specification "ResourceType=instance,Tags=[{Key=Name, Value=\"$NAMETAG-instances\"},{Key=owner, Value=\"$OWNERTAG\"}, {Key=expire-on,Value=\"$EXPIREON\"}, {Key=purpose,Value=\"$PURPOSETAG\"}]" --profile helene.brulin > /dev/null

sleep 1
count=$(aws ec2 describe-instances --filters "Name=tag:owner,Values=$OWNERTAG" "Name=tag:Name,Values=$NAMETAG-instances" "Name=instance-state-name,Values=running" --profile helene.brulin | jq -r '.Reservations[0].Instances | length')
...
for inst in $(aws ec2 describe-instances --filters "Name=tag:owner,Values=$OWNERTAG" "Name=tag:Name,Values=$NAMETAG-instances" "Name=instance-state-name,Values=running" --profile helene.brulin | jq -r '.Reservations[].Instances[].PublicDnsName');
do
...
sudo yum install -y http://$PRIVDNS:8080/download/agent/automation/mongodb-mms-automation-agent-manager-latest.x86_64.rhel7.rpm
...
echo "mmsAgentName=\$PRIVATE_IP" | sudo tee -a /etc/mongodb-mms/automation-agent.config
