...
#OM_VERSION=https://downloads.mongodb.com/on-prem-mms/rpm/mongodb-mms-8.0.17.500.20251204T1318Z.x86_64.rpm 
#APP_VERSION=https://repo.mongodb.org/yum/amazon/2023/mongodb-org/8.0/x86_64/RPMS/mongodb-org-server-8.0.9-1.amzn2023.x86_64.rpm

OM_VERSION=https://downloads.mongodb.com/on-prem-mms/rpm/mongodb-mms-8.0.19.500.20260114T0939Z.x86_64.rpm
APP_VERSION=https://repo.mongodb.org/yum/amazon/2023/mongodb-org/8.2/x86_64/RPMS/mongodb-org-server-8.2.4-1.amzn2023.x86_64.rpm


#APP_VERSION=https://repo.mongodb.org/yum/amazon/2023/mongodb-org/8.2/x86_64/RPMS/mongodb-org-server-8.2.2-1.amzn2023.x86_64.rpm
...
aws ec2 run-instances --image-id $IMAGE --count 1 --instance-type t3a.xlarge --key-name $KEYNAME \
  ...
  --tag-specification "ResourceType=instance,Tags=[{Key=Name, Value=\"$NAMETAG-om\"},{Key=owner, Value=\"$OWNERTAG\"}, {Key=expire-on,Value=\"$EXPIREON\"}, {Key=purpose,Value=\"$PURPOSETAG\"}]" --profile helene.brulin > /dev/null

echo "Done"
# wait a couple seconds that the instance is up
sleep 10

res=$(aws ec2 describe-instances --filters "Name=tag:owner,Values=$OWNERTAG" "Name=tag:Name,Values=$NAMETAG-om" "Name=instance-state-name,Values=running" --profile helene.brulin)
...
sudo yum install -y $APP_VERSION
...
./launch-hosts.sh $PRIVDNS $PROJECT_ID $AGENT_API_KEY
