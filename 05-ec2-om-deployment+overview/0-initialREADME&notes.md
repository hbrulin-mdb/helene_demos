...
- three for demo instances (t3a.xlarge)
...
- ops manager (mms)
- APPDB - set of mongod, a replicaset that allows ops manager to run and be HA - APPDB is the database used by the Ops Manager Application.
- automation agent

Load Balancer for Ops Manager
- For the Ops Manager web UI and HA across multiple application servers, organizations commonly use a load balancer. This applies to the Ops Manager application tier, not to the mongod/mongos database processes. -> it is best to install ops manager on all three servers, and not on a standalone machine like in this demo. The MongoDB University Course "Introduction to Ops Manager" does that.
- When you should not use a load balancer: In front of replica set members (mongod). Drivers need direct member access for topology discovery and elections.

For prod deployments, it is recommended to deploy ops manager on at least 3 servers behind a LB
Can a 3 machine deployment of ops manager, be the same machines as the mongodb cluster it manages? NO. it’s technically possible, but strongly discouraged, and not appropriate for production.

## Notes on APP_DB
I have a project called APP_DB - it is empty. It's normal, because Ops Manager is not managing, by default, its own AppDB replicaset. 
-> By installing the Ops Manager App DB Monitoring, it can be done. 